  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold text-primary">สถานะเงินบำรุง</h1>
        <p className="text-muted-foreground">รายรับ-รายจ่าย และเงินคงเหลือรายเดือน</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="card-hover border-l-4 border-l-primary">
          <CardContent className="flex items-center gap-4 p-6">
            ...
            <div>
              <p className="text-sm text-muted-foreground">ยอดคงเหลือ (ล่าสุด)</p>
      ...
      <Card className="card-hover">
        <CardHeader>
          <CardTitle>รายละเอียดเงินบำรุงรายเดือน</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-left py-3 px-4 font-medium">เดือน</th>
                  <th className="text-left py-3 px-4 font-medium">สถานบริการ</th>
                  <th className="text-right py-3 px-4 font-medium">รายรับ</th>
                  <th className="text-right py-3 px-4 font-medium">รายจ่าย</th>
              ...
              <tbody>
                {filteredFinance.map((record) => (
                  <tr key={record.id} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 font-medium">{record.unitName}</td>
                    ...
                    <td className="py-3 px-4 text-right font-medium">{formatCurrency(record.balance)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Dialog open={modal.open} onOpenChange={(open) => (!open ? setModal({ open: false }) : null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {modal.open ? (modal.type === "income" ? "รายการรายได้" : "รายการรายจ่าย") : ""}
            </DialogTitle>
            <DialogDescription>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-background">
                  <tr className="border-b text-left text-muted-foreground">
                    <th className="px-4 py-3 font-medium">Accountname</th>
